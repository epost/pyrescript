...
--
module Language.PyreScript.Pretty.Py
  ( prettyPrintJS
...
literals = mkPattern' match'
  ...
  match (JSObjectLiteral _ ps) = mconcat <$> sequence
    [ return $ emit "{\n"
    , withIndent $ do
        jss <- forM ps $ \(key, value) -> fmap ((emit (show key) <> emit ": ") <>) . prettyPrintJS' $ value
  ...
  operators =
    OperatorTable [ [ Wrap accessor $ \prop val -> val <> emit "." <> prop ]
                  ...
                  , [ binary    LessThan             "<"
                    ...
                    , AssocR instanceOf $ \v1 v2 ->
                      emit "isinstance(" <> v1 <> emit ", " <> v2 <> emit ")" ]
                  , [ binary    EqualTo              "=="
                    , binary    NotEqualTo           "!=" ]
                  , [ binary    BitwiseAnd           "&" ]
                  , [ binary    BitwiseXor           "^" ]
                  ...
                  , [ binary    And                  " and " ]
                  , [ binary    Or                   " or " ]
                  , [ Wrap conditional $ \(ss, th, el) cond ->
                      prettyPrintJS1 th <> addMapping' ss <>
                      emit " if " <> cond <> addMapping' ss <>
                      emit " else " <> prettyPrintJS1 el ]
                    ]
